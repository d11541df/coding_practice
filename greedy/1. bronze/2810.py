...
1 초	128 MB	13424	7342	6382	54.762%
문제
십년이면 강산이 변한다.
...
while i < len(seats):
    if seats[i] == 'L':
        # L은 항상 두 개씩 주어지므로, 한 쌍으로 처리
        couple_count += 1
        i += 2
    else:
        i += 1

holders = (N + 1) - couple_count
result = min(holders, N)
print(result)
...
seat_stack = deque(seats)
couple_count = 0

while seat_stack:
    seat = seat_stack.popleft()
    if seat == 'L':
        # 커플석이므로 다음 좌석도 반드시 L이다.
        couple_count += 1
        if seat_stack:
            seat_stack.popleft()  # 다음 L은 같이 처리
    # S인 경우는 그냥 넘어감
